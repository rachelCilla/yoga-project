...
export default function PosesByDifficulty({ showDifficulty }) {
	...
	return (
		<div className="poses-by-difficulty">
			<h1
				// className={`${posesBy} ${isHovered ? hovered : ""}`}
			...
			{isHovered && (
				<>
					...
					<button onClick={() => handleButtonClick("intermediate")}>Intermediate Poses</button>
					<button onClick={() => handleButtonClick("expert")}>Expert Poses</button>
				</>
			)}
